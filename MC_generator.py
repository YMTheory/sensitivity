...
class MC_generator:
    def __init__(self, source, det, dm2=1.0, sin2theta_square=0.1, int_type='ES', seed=42, binning_file='/p/lustre1/yu47/Sterile_Neutrino/sensitivity/config/binning_3cm.json'):
        ...
        #self.n_ES       = 60605
        #self.n_2nbb     = 65990
        #self.n_solarpp  = 280.5
        #self.n_Kr85     = 504.7
        #self.n_Rn222    = 1227

        self.es_signal = signal_calculation()
        self.es_signal.load_ESspectrum()
    ...
    def sample_electron_energy_ES(self, Temin=0, Temax=0.7, n_samples=10000, Ev=0.75):
        E_vals = np.linspace(Temin, Temax, 1000)
        dcs_values = self.es_signal.energy_spectrum_ES(E_vals)
        cdf = np.cumsum(dcs_values)
        cdf /= cdf[-1]
    
        random_numbers = np.random.rand(n_samples)
        energies = np.interp(random_numbers, cdf, E_vals)
    ...
    def sample_electron_energy_bkg(self, Temin=0, Temax=0.7, n_samples=10000, Ev=0.75):
        ...
        random_numbers = np.random.rand(n_samples)
        energies = np.interp(random_numbers, cdf, Evals)
    ...
    def monte_carlo_sampling_ESbkg(self, n_event=1e5):
        ...
        energies = self.sample_electron_energy_bkg(n_samples=n_event)
    ...
    def monte_carlo_sampling(self, n_event=1e5, return_pos=False, int_type='CC'):
        ...
        if int_type == 'ES':
            Te = self.sample_electron_energy_ES(n_samples=n_event)
            Te = np.reshape(Te, (len(Te), 1))
            pairs = np.hstack((pairs, Te))
        #return pairs 
        if return_pos:
