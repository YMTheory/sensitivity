...
def MakePlots(outname,logscale=False):
    ...
    min_x, max_x = 0.25, 5.25 
    if logscale:
        min_x, max_x = 0.4, 6
    min_y, max_y = 1e-4, 1e2
    ...
    yList = {}
    for roi in roiList:
        yList[roi] = [results[resol][roi] for resol in sorted(results)]
    ...
    plt.xlabel(r"Resolution $\sigma/E$ at the Q-value [%]", fontsize=axis_label_fontsize)
    # plt.ylabel(r"$^{136}$Xe  $2\nu\beta\beta$ Background [cts/(tonne$\cdot$yr)]", fontsize=axis_label_fontsize)
    plt.ylabel(r"$^{136}$Xe  $2\nu\beta\beta$ [cts/(ROI$\cdot$tonne$\cdot$yr)]", fontsize=axis_label_fontsize)
...
if __name__ == "__main__":
    ...
    MakePlots("bb2n_bkg_vs_resol_v2_lin",logscale=False)
