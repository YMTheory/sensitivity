...
class ExcelTableReader:
    ...
    def __init__(self):
        self.filename = '../tables/Summary_01042016_original_v2.xlsx'
        ...
        self.counts_sheet = {'name': '%s_ExpectedCounts', 'startRow': 6, 'componentColumn': 'A', 'isotopeColumn': 'B', 'cvColumn': {'SS': 'AR', 'MS': 'AH'}, 'errorColumn': {'SS': 'AS', 'MS': 'AI'}}
        self.hiteff_sheet = {'name': 'MC_RawCounts_%s_Integrals', 'startRow': 6, 'componentColumn': 'A', 'isotopeColumn': 'B', 'nColumn': 'C', 'kColumn': 'AH'}
        self.constants_sheet = {'name': 'Constants', 'startHLRow': 16, 'endHLRow': 24, 'isotopeColumn': 'A', 'hlColumn': 'B'}
    ...
    def ReadCounts(self,suffix):
        ...
        for row in range(start_row,end_row):
            ...
            pdf = self.GetPdfName(component,isotope)
            print row, '/', end_row, ':', component, isotope, pdf
            counts[pdf] = {'cv':self.GetCellValue(ws,sheet['cvColumn'][suffix],row),'error':self.GetCellValue(ws,sheet['errorColumn'][suffix],row)}
...
class RootTreeWriter():
    ...
    def __init__(self):
        self.filename = 'test_original_tree.root'

        self.pdf_filename_pattern = '/data/data033/exo/software/nEXO_Sensitivity/quick/v0/histos/individual_histos/nEXO_Histos_%s.root'
