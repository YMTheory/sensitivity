...
if __name__ == "__main__":
    ...
    magic_files = [f for f in listdir(options.magic_number_dir) if isfile(join(options.magic_number_dir, f))]
    print(magic_files)
    for f in magic_files:
        numcounts = float(f.split('_')[-3])
        if numcounts>=15: continue
        outfile = ROOT.TFile(join(options.magic_number_dir, f))
