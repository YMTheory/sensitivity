...
if len(sys.argv) != 6:
	...
	print('\tpython Compute90PercentLimit_PythonCode.py ' + \
		'<iteration_num> <num_datasets_to_generate> <critical_lambda_scale_factor> <critical_lambda_file> <output_directory>\n\n')
	sys.exit()

iteration_num = int(sys.argv[1])
num_datasets = int(sys.argv[2])
critical_lambda_scale_factor = float(sys.argv[3])
...
SplineFunc = interp1d(critical_lambda_data[:,0],critical_lambda_data[:,1])
...
initial_guess[ likelihood.GetVariableIndex('Bb2n') ] = \
        initial_guess[ likelihood.GetVariableIndex('Bb2n') ] * critical_lambda_scale_factor
...
output_df.to_hdf('{}/ba_tagging_sens_output_file_90CL_scaling_{:>05.5}_nodataset_{}.h5'.format(\
                             output_dir,critical_lambda_scale_factor,iteration_num),key='df')
