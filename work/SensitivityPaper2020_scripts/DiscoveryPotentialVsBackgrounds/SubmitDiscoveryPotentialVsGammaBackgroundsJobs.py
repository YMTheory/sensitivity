...
executable_name = 'Compute_DiscoveryPotentialVsGammaBackgrounds.py'
...
for livetime in Livetimes[::-1]: 
	# for bkg_scale_factor in [0.01, 0.1, 0.3, 1, 3, 10] : 
	for bkg_scale_factor in [0.01, 0.1, 10.0] : 
		for bb0n_count in bb0n_counts[livetime][::-1]:
			print(livetime, bkg_scale_factor, bb0n_count, num_jobs[bb0n_count], num_datasets_per_job[bb0n_count])
			...
			for iter_num in np.arange(iter_start, iter_start + num_jobs[bb0n_count], 1):
				...
				scriptfilename = outputdir + 'sub/' + base + "_{:03}_g{:03}.sub".format(iter_num, bkg_scale_factor)
				os.system( "rm -f " + scriptfilename )
				outfilename = outputdir + 'out/' + base + "_{:03}_g{:03}.out".format(iter_num, bkg_scale_factor)
				...
				if CLUSTER=='LLNL':
					thescript = "#!/bin/bash\n" + \
						...
						"python3 DiscoveryPotential/" + executable_name + " {} {} {} {} {} {} {} {} {} \n".format(iter_num, bkg_shape_err, num_datasets_per_job[bb0n_count], components_table, outputdir+'h5/', bb0n_count, livetime, yaml_card, bkg_scale_factor) + \
