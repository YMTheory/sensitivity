...
# for database in ['023', '024', '025']:
    ...
    livetime_list = [0.5, 1, 2, 5, 10]
    for lt in livetime_list:
        resolution = '0.008'
        for num in range(0, 100):
            basename = base + str(num)
            if not os.path.exists(outputdir + "/Sub/" + date):
                    os.makedirs(outputdir + "/Sub/" + date)
            scriptfilename = outputdir + "/Sub/" + date + "/" + base + str(lt) + "_" + str(num) + ".sub"
            os.system("rm -f " + scriptfilename)
            if not os.path.exists(outputdir + "/Out/" + date):
                    os.makedirs(outputdir + "/Out/" + date)
            outfilename = outputdir + "/Out/" + date + "/" + base + str(lt) + "_" + str(num) + ".out"
            os.system("rm -f " + outfilename)
            errfilename = outputdir + "/Err/" + base + str(lt) + "_" + str(num) + ".err"
            ...
            thescript = "#!/bin/bash\n" + \
                    ...
                    "python3 " + execdir + "/Eres_study/Livetime_Lambda_Sensitivity.py {} {} {} {} {} {} {} {} {} {} {} {}" \
                                           "\n".format(num, bkg_shape_err, num_datasets, outputdir, num_hypotheses,
                                               date, config_loc, comp_loc, crit_lam_loc, resolution, USE_WILKS, lt) + \
