...
bkg_scale_factor = float(sys.argv[5])
...
for index, row in workspace.df_components.iterrows():
	...
	if row['PDFName'].split('_')[0] in isotopes_to_scale:
		...
		workspace.df_components.loc[index,'SpecActiv'] = \
						bkg_scale_factor * row['SpecActiv']
		workspace.df_components.loc[index,'SpecActivErr'] = \
						bkg_scale_factor * row['SpecActivErr']
...
print('\n****************************************************************************************')
print('Variable list after scaling backgrounds:')
...
for j in range(0,num_datasets):
	...
	for i in (range(0,num_hypotheses)):
		...
		if bkg_scale_factor <0.3:
			signal_hypothesis = float(i)*1.+0.000001
		elif bkg_scale_factor < 1.:
			signal_hypothesis = float(i)*1.5 + 0.000001
		else: 
...
output_df.to_hdf('{}/sens_output_file_gamma_backgrounds_study_{:0>4.4}x_90CL_{:03}_D023.h5'.format(\
                         output_dir, bkg_scale_factor, job_id_num ),\
                         key='df')
