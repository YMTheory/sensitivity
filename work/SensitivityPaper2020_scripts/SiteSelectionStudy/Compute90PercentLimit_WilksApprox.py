...
def get_parser():
    ...
    parser.add_argument("-l", "--label", type=str, default="", help='Job label')
...
#             workspace.df_components.loc[index, 'SpecActivErr'] = args.bkg_scale_factor * row['SpecActivErr']
        ...
        if ('Xe137' in row['PDFName']) and workspace.Xe137Sample:
            ...
            workspace.df_components.loc[index, 'SpecActiv'] = workspace.mean_xe137_spec_activ
            workspace.df_components.loc[index, 'SpecActivErr'] = workspace.mean_xe137_spec_activ
    ...
    output_df.to_hdf(f'{args.output_dir}/'
                     f'sens_output_file_{args.label}_'
                     f'{args.job_id_num:03}.h5',
                     key='df')
