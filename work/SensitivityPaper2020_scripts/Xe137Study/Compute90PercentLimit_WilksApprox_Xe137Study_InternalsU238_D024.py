...
internalsu238_scale_factor = float(sys.argv[6])
...
# Scale the InternalsU238 components.
for index, row in workspace.df_components.iterrows():
	if 'Internals_U238' in row['Group']:
		workspace.df_components.loc[index,'SpecActiv'] = \
						internalsu238_scale_factor * row['SpecActiv']
		workspace.df_components.loc[index,'SpecActivErr'] = \
						internalsu238_scale_factor * row['SpecActivErr']
...
for j in range(0,num_datasets):
	...
	for i in (range(0,num_hypotheses)):
		...
		if internalsu238_scale_factor > 9.:
...
output_df.to_hdf('{}/sens_output_file_xe137study_internalsu238_{:0>4.4}x_90CL_{:03}_D024.h5'.format(\
                         output_dir, internalsu238_scale_factor, job_id_num ),\
                         key='df')
