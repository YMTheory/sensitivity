...
outputname = ""
executable_name = 'Compute90PercentLimit_WilksApprox_Xe137Study_VesselU238_D024.py'
...
components_table = '/p/vast1/nexo/sensitivity2020/pdfs/component_tables/'+\
			'ComponentsTable_D-024_merged-v10b_Optimized_DNN_Standoff_Binning_version1.h5'

vesselu238_scale_factor = 100.
...
base = "Run_vesselu238_x"

for iter_num in range(jobs_offset,num_jobs+jobs_offset):
		
		scriptfilename = outputdir +  base + '{:0>4.4}'.format(vesselu238_scale_factor) + "_{:03}.sub".format(iter_num)
		os.system( "rm -f " + scriptfilename )
		outfilename = outputdir + base + '{:0>4.4}'.format(vesselu238_scale_factor) + "_{:03}.out".format(iter_num)
		...
		thescript = "#!/bin/bash\n" + \
			...
			"python3 SensitivityPaper2020_scripts/Xe137Study/" + executable_name +\
	                                                ...
	                                                num_datasets_per_job, components_table, \
                                                        outputdir, vesselu238_scale_factor) + \
