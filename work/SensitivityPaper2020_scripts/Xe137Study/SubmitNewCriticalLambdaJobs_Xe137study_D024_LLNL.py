...
import os
execdir = "/g/g20/lenardo1/nEXO/sensitivity/work/SensitivityPaper2020_scripts/Xe137Study/"

outputdir = "/p/lustre2/lenardo1/sensitivity_output/Mar7_Xe137_CriticalLambda_D024/"
...
base = "Xe137Study_CritLambda_Sensitivity2020_"


# Number of toy datasets to run for each hypothesis
num_datasets=500

xe137_scale_factor = 3.
offset = 0
...
for num in range(0,1000):
	...
	scriptfilename = outputdir +  base + str(num) + ".sub"
	os.system( "rm -f " + scriptfilename )
	outfilename = outputdir + base + str(num) + ".out"
	...
	thescript = "#!/bin/bash\n" + \
		...
		"python3 ComputeCriticalLambdaForFixedNumSignal_Xe137study_D024.py " + \
				"{} ".format(iteration_num) + \
				"{} ".format(hyp_val) + \
				"{} ".format(num_datasets) + \
				"{} ".format(outputdir) + \
				"{} ".format(components_table) + \
				"{} ".format(xe137_scale_factor) + "\n" + \
