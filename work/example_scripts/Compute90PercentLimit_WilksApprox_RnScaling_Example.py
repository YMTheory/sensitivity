...
for j in range(0,num_datasets):
	...
	fixed_fit_converged_array = np.array([],dtype=bool)
	fixed_fit_covar_array = np.array([],dtype=bool)
	crossing = -1
	output_row = dict()
	...
	likelihood.AddPDFDataframeToModel( workspace.df_group_pdfs, \
        ...
        # Scale the Rn component according to the multiplicative factor passed as a argument
	rn222_idx = likelihood.model.GetVariableIndexByName('Rn222')
	...
	for i in (range(0,num_hypotheses)):
		...
		xvals[i] = signal_hypothesis
	        ...
	        ###########################################################################
		# All the exciting stuff happens here! Note that signal expectation = 0
		...
		fixed_fit_converged_array = np.append(fixed_fit_converged, lambda_fit_result['fixed_fit_converged'])
		fixed_fit_covar_array = np.append(fixed_fit_covar, lambda_fit_result['fixed_fit_covar'])
	...
	if lambda_fit_result['best_fit_covar']:
		...
		if DEBUG_PLOTTING:
			...
			plt.plot( xvals[fixed_fit_converged_array],\
					lambdas[fixed_fit_converged_array],\
        ...
        # Put the fit results into a dict, which will get saved in a pandas DataFrame
	output_row['num_signal'] = xvals
	output_row['lambda'] = lambdas
	output_row['fixed_fit_converged'] = fixed_fit_converged_array
	output_row['fixed_fit_acc_covar'] = fixed_fit_covar_array
